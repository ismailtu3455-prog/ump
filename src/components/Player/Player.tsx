...
function Player() {
  ...
  const { windowSettings } = useAppSelector(state => state.ui);
  ...
  // Добавлено: Состояние для анимации Play/Pause по центру
  const [centerIconIndicator, setCenterIconIndicator] = useState<'play' | 'pause' | null>(null);
  const centerIconTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Инициализация Canvas один раз для оптимизации памяти
  useEffect(() => {
  ...
  const togglePlayPause = useCallback(() => {
    // Добавлено: отображение анимации по центру
    const newStateIsPlaying = !isPlaying;
    setCenterIconIndicator(newStateIsPlaying ? 'play' : 'pause');
    
    if (centerIconTimeoutRef.current) {
      clearTimeout(centerIconTimeoutRef.current);
    }
    centerIconTimeoutRef.current = setTimeout(() => {
      setCenterIconIndicator(null);
    }, 500);
  ...
  const togglePiP = useCallback(async () => {
    ...
    try {
      ...
      dispatch(addNotification({ title: 'Ошибка PiP', message: 'Не удалось переключить режим', type: 'error' }));
    }
  }, [dispatch]);
    ...
    if (ctx) {
      try {
        ...
        if (window.electronAPI?.saveScreenshot) {
          const result = await window.electronAPI.saveScreenshot(activePlaylistId || 'recent', base64Data);
          if (result.success) {
            dispatch(addNotification({ title: 'Скриншот сохранен', message: result.path, type: 'success' }));
          } else {
            throw new Error(result.error);
          }
        }
      } catch (err) {
        dispatch(addNotification({ title: 'Ошибка', message: 'Не удалось сделать скриншот', type: 'error' }));
      }
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      const hotkey = windowSettings?.screenshotHotkey || 'F11';
  ...
  }, [dispatch, togglePlayPause, handleFullscreen, takeScreenshot, windowSettings?.screenshotHotkey]);
  ...
  return (
    ...
    >
      ...
      {/* Анимация Play/Pause по центру видео */}
      {centerIconIndicator && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
          <div className="bg-black/50 text-white rounded-full p-4 animate-ping-once transition-all duration-500 opacity-0 scale-150">
            {centerIconIndicator === 'play' ? (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <select
                ...
                className="bg-dark-700 hover:bg-dark-600 border border-dark-500 rounded px-3 py-1.5 text-sm text-white cursor-pointer focus:outline-none focus:ring-2 focus:ring-primary-500"
              >
              ...
              {/* Screenshot */}
              <button onClick={takeScreenshot} className="p-2 hover:bg-white/10 rounded-full transition-colors" title={`Скриншот (${windowSettings?.screenshotHotkey || 'F11'})`}>
              ...
              {/* PiP */}
              <button onClick={togglePiP} className="p-2 hover:bg-white/10 rounded-full transition-colors" title="Картинка в картинке">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {/* Fullscreen */}
              <button onClick={handleFullscreen} className="p-2 hover:bg-white/10 rounded-full transition-colors" title="На весь экран (F)">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
