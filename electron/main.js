...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    icon: path.join(basePath, isDev ? '../public/icon.png' : '../build/icon.png'),
...
function setupIpcHandlers() {
  ...
  // Сохранение скриншота (Папка скриншотов создается параллельно папкам плейлистов)
  ipcMain.handle('save-screenshot', async (event, playlistId, base64Data) => {
    try {
      const screenshotsDir = path.join(PLAYLISTS_DIR, 'Screenshots');
      ...
      // Очистка от префикса base64 браузера для предотвращения повреждения изображения
